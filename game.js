...
class NPC extends GameObject {
    constructor(lane) {
        // Enhanced car type selection with more variety
        const carTypes = [
            ...
            { type: 'sedan', width: 82, height: 128, color: '#e67e22' }, // Orange sedan
            { type: 'coupe', width: 78, height: 118, color: '#8e44ad' }, // Purple coupe
            { type: 'wagon', width: 88, height: 135, color: '#16a085' }, // Green wagon
            { type: 'van', width: 92, height: 145, color: '#c0392b' } // Red van
        ];
        ...
        // Assign a specific car image to ensure each car looks different
        const carImages = ['npcCar', 'npcCar2', 'npcCar3'];
        this.carImageKey = carImages[Math.floor(Math.random() * carImages.length)];
    ...
    update(dt, player) {
        ...
        if (player) {
            ...
            if (distanceToPlayer < this.avoidanceDistance && horizontalDistance < 80) {
                ...
                // Limit how far NPC can move from original position (reduced for better track staying)
                this.avoidanceOffset = Math.max(-40, Math.min(40, this.avoidanceOffset));
        ...
        // Apply movement: wobble + avoidance + lane change (reduced wobble for better track staying)
        ...
        // Keep car on track - constrain to lane boundaries
        const trackLeftBoundary = 50;  // Left edge of the road
        const trackRightBoundary = 350; // Right edge of the road
        this.x = Math.max(trackLeftBoundary + this.width / 2, Math.min(trackRightBoundary - this.width / 2, this.x));
...
class GameManager {
    ...
    loadImages() {
        ...
        const imageList = {
            ...
            // NPC car images - one unique SVG for each car type
            saloonCar: 'assets/saloon-car.svg',
            suvCar: 'assets/suv-car.svg',
            hatchbackCar: 'assets/hatchback-car.svg',
            sportsCar: 'assets/sports-car.svg',
            truckCar: 'assets/truck-car.svg',
            compactCar: 'assets/compact-car.svg',
            sedanCar: 'assets/sedan-car.svg',
            coupeCar: 'assets/coupe-car.svg',
            wagonCar: 'assets/wagon-car.svg',
            vanCar: 'assets/van-car.svg',
...
4. TEST YOUR IMAGES:
   ...
   - Fallback shapes will show if images fail to load
    ...
    drawNPC(npc) {
        ...
        // Try to use all available car images for maximum variety
        const availableImages = ['npcCar', 'npcCar2', 'npcCar3'].filter(key => 
            this.images[key] && this.images[key].complete
        );
        
        if (availableImages.length > 0) {
            // Use the specific car image assigned to this NPC for consistent appearance
            const selectedImageKey = npc.carImageKey || availableImages[0];
            
            // If the assigned image isn't available, fall back to a different one
            if (!this.images[selectedImageKey] || !this.images[selectedImageKey].complete) {
            ...
            // Fallback to geometric shapes if no images loaded
    ...
    getCarTypeIndex(carType) {
        // Map car types to different image indices for maximum variety
        const typeMap = {
            'saloon': 0,      // Uses npcCar (npc-car.svg)
            'suv': 1,         // Uses npcCar2 (npc-car-2.svg)
            'hatchback': 2,   // Uses npcCar3 (!player-car.svg)
            'sports': 0,      // Uses npcCar (npc-car.svg)
            'truck': 1,       // Uses npcCar2 (npc-car-2.svg)
            'compact': 2      // Uses npcCar3 (!player-car.svg)
        };
        return typeMap[carType] || 0;
    }
